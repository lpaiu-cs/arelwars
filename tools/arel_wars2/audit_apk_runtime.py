...
DEFAULT_OUTPUT = Path("recovery/arel_wars2/native_tmp/apk_runtime_audit.json")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Audit the current Arel Wars 2 APK for x86_64 packaging readiness and JNI/native blockers.",
    )
...
def build_readiness_summary(workspace: dict, apk_zip: dict, dex: dict, native_libs: dict) -> dict:
    ...
    if set(libs_by_abi.keys()).issubset({"armeabi", "armeabi-v7a"}) and "x86_64" not in libs_by_abi:
        blockers.append("APK ships only ARM32 native ABIs (`armeabi` / `armeabi-v7a`), so an x86_64 replacement runtime is still required.")
